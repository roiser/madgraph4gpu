...
// ** NB1 Throughputs (e.g. 6.8E8) are events/sec for "./gcheck.exe -p 65536 128
// 12"
// ** NB2 Baseline on b7g47n0004 fluctuates (probably depends on load on other
// VMs)

// Choose how random numbers are generated
// If one of these macros has been set from outside with e.g.
// -DMGONGPU_COMMONRAND_ONHOST, nothing happens
#if not defined MGONGPU_CURAND_ONDEVICE and                                    \
    not defined MGONGPU_CURAND_ONHOST and                                      \
    not defined MGONGPU_COMMONRAND_ONHOST
// Curand random number generation (CHOOSE ONLY ONE)
#define MGONGPU_CURAND_ONDEVICE                                                \
  1 // default (curand: CUDA on device, C++ on host)
//#define MGONGPU_CURAND_ONHOST 1 // (curand: CUDA on host, C++ on host)
//#define MGONGPU_COMMONRAND_ONHOST 1 // (common rand: CUDA on host, C++ on
//host)
#endif

// Choose floating point precision
// If one of these macros has been set from outside with e.g.
// -DMGONGPU_FPTYPE_FLOAT, nothing happens (issue #167)
...
// This optimization can gain almost a factor 4 in C++, similar to -flto (issue
// #229) By default, do not inline, but allow this macro to be set from outside
// with e.g. -DMGONGPU_INLINE_HELAMPS
//#undef MGONGPU_INLINE_HELAMPS // default
////#define MGONGPU_INLINE_HELAMPS 1

// Choose whether to hardcode the cIPC/cIPD physics parameters rather than
// reading them from user cards This optimization can gain 20% in CUDA in eemumu
// (issue #39) By default, do not hardcode, but allow this macro to be set from
// outside with e.g. -DMGONGPU_HARDCODE_CIPC
...
// Cuda nsight compute (ncu) debug: add dummy lines to ease SASS program flow
// navigation
#ifdef __CUDACC__
...
const int npari = 2; // #particles in the initial state (incoming): e.g. 2 (e+
                     // e-) for e+ e- -> mu+ mu-
const int nparf = 2; // #particles in the final state (outgoing): e.g. 2 (mu+
                     // mu-) for e+ e- -> mu+ mu-
const int npar = npari + nparf; // #particles in total (external = initial +
                                // final): e.g. 4 for e+ e- -> mu+ mu-

const int ncomb = 16; // #helicity combinations: e.g. 16 for e+ e- -> mu+ mu-
                      // (2**4 = fermion spin up/down ** npar)

const int nw6 = 6; // dimensions of each wavefunction (HELAS KEK 91-11): e.g. 6
                   // for e+ e- -> mu+ mu- (fermions and vectors)
const int nwf = 5; // #wavefunctions = #external (npar) + #internal: e.g. 5 for
                   // e+ e- -> mu+ mu- (1 internal is gamma or Z)

// --- Platform-specific software implementation details

// Maximum number of blocks per grid
// ** NB Some arrays of pointers will be allocated statically to fit all these
// blocks
// ** (the actual memory for each block will then be allocated dynamically only
// for existing blocks)
// const int nbpgMAX = 2048;

// Maximum number of threads per block
// const int ntpbMAX = 256; // AV Apr2021: why had I set this to 256?
const int ntpbMAX = 1024; // NB: 512 is ok, but 1024 does fail with "too many
                          // resources requested for launch"

// Alignment requirement for using reinterpret_cast with SIMD vectorized code
// (using reinterpret_cast with non aligned memory may lead to segmentation
// faults!)
#ifndef __CUDACC__
constexpr int cppAlign = 32; // 64; // alignment requirement for SIMD
                             // vectorization (64-byte i.e. 512-bit)
#endif
...
#endif
#else
// "512y" AVX512 with 256 width (256-bit ie 32-byte): 4 (DOUBLE) or 8 (FLOAT)
// [gcc DEFAULT]
#ifdef MGONGPU_FPTYPE_DOUBLE
...
#ifdef __CUDACC__
// -----------------------------------------------------------------------------------------------
// --- GPUs: neppM is best set to a power of 2 times the number of fptype's in a
// 32-byte cacheline
// --- This is relevant to ensure coalesced access to momenta in global memory
// --- Note that neppR is hardcoded and may differ from neppM and neppV on some
// platforms
// -----------------------------------------------------------------------------------------------
// const int neppM = 64/sizeof(fptype); // 2x 32-byte GPU cache lines (512
// bits): 8 (DOUBLE) or 16 (FLOAT)
const int neppM = 32 / sizeof(fptype); // (DEFAULT) 32-byte GPU cache line (256
                                       // bits): 4 (DOUBLE) or 8 (FLOAT)
// const int neppM = 1;  // *** NB: this is equivalent to AOS ***
#else
// -----------------------------------------------------------------------------------------------
// --- CPUs: neppM is best set equal to the number of fptype's (neppV) in a
// vector register
// --- This is relevant to ensure faster access to momenta from C++ memory cache
// lines
// --- However, neppM is now decoupled from neppV (issue #176) and can be
// separately hardcoded
// --- In practice, neppR, neppM and neppV could now (in principle) all be
// different
// -----------------------------------------------------------------------------------------------
#ifdef MGONGPU_CPPSIMD
const int neppM =
    MGONGPU_CPPSIMD; // (DEFAULT) neppM=neppV for optimal performance
// const int neppM = 64/sizeof(fptype); // maximum CPU vector width (512 bits):
// 8 (DOUBLE) or 16 (FLOAT) const int neppM = 32/sizeof(fptype); // lower CPU
// vector width (256 bits): 4 (DOUBLE) or 8 (FLOAT) const int neppM = 1; // ***
// NB: this is equivalent to AOS *** const int neppM = MGONGPU_CPPSIMD*2; // FOR
// TESTS
#else
const int neppM = 1; // (DEFAULT) neppM=neppV for optimal performance (NB: this
                     // is equivalent to AOS)
#endif
#endif
...
// Cuda nsight compute (ncu) debug: add dummy lines to ease SASS program flow
// navigation Arguments (not used so far): text is __FUNCTION__, code is 0
// (start) or 1 (end)
